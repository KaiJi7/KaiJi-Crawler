...
class Crawler(object):
    def __init__(self, game_type):
        ...
        self.prediction = {cl_const.all_member: self.prediction_info_all_member,
    ...
    def start_crawler(self, start_date, end_date):
        ...
        start_date = datetime.datetime.strptime(start_date, self.config['crawler']['dateFormat'])
        end_date = datetime.datetime.strptime(end_date, self.config['crawler']['dateFormat'])
        ...
        for date in pd.date_range(start=start_date, end=end_date):
            ...
            for prediction_group in cl_const.prediction_group.keys():
                res = requests.get(self.get_url(date, cl_const.prediction_group[prediction_group]))
                soup = BeautifulSoup(res.text, 'html.parser')
                if prediction_group == cl_const.all_member:
                    # get game info for once
                    self.get_game_data(date, soup)
                    self.write_to_db(pd.DataFrame.from_dict(self.game_info), db_const.game_data)
        ...
        self.logger.info(
            'crawler task done, total crawled games: {}, days: {}'.format(total_crawled_game,
    ...
    def append_prediction_national_point_spread(self, row_content, guest_row, group):
        ...
        if guest_row:
            self.prediction[group][db_const.percentage_national_point_spread_guest].append(percentage)
            self.prediction[group][db_const.population_national_point_spread_guest].append(population)
        else:
            self.prediction[group][db_const.percentage_national_point_spread_host].append(percentage)
            self.prediction[group][db_const.population_national_point_spread_host].append(population)
        return
    ...
    def append_prediction_national_total_point(self, row_content, guest_row, group):
        ...
        percentage, population = date if len(date) == 2 else (0, 0)
        if guest_row:
            self.prediction[group][db_const.percentage_national_total_point_over].append(percentage)
            self.prediction[group][db_const.population_national_total_point_over].append(population)
        else:
            self.prediction[group][db_const.percentage_national_total_point_under].append(percentage)
            self.prediction[group][db_const.population_national_total_point_under].append(population)
        return
    ...
    def append_prediction_local_point_spread(self, row_content, guest_row, group):
        ...
        percentage, population = date if len(date) == 2 else (0, 0)
        if guest_row:
            self.prediction[group][db_const.percentage_local_point_spread_guest].append(percentage)
            self.prediction[group][db_const.population_local_point_spread_guest].append(population)
        else:
            self.prediction[group][db_const.percentage_local_point_spread_host].append(percentage)
            self.prediction[group][db_const.population_local_point_spread_host].append(population)
        return
    ...
    def append_prediction_local_total_point(self, row_content, guest_row, group):
        ...
        percentage, population = date if len(date) == 2 else (0, 0)
        if guest_row:
            self.prediction[group][db_const.percentage_local_total_point_over].append(percentage)
            self.prediction[group][db_const.population_local_total_point_over].append(population)
        else:
            self.prediction[group][db_const.percentage_local_total_point_under].append(percentage)
            self.prediction[group][db_const.population_local_total_point_under].append(population)
        return
    ...
    def append_prediction_local_original(self, row_content, guest_row, group):
        ...
        percentage, population = date if len(date) == 2 else (0, 0)
        if guest_row:
            self.prediction[group][db_const.percentage_local_original_guest].append(percentage)
            self.prediction[group][db_const.population_local_original_guest].append(population)
        else:
            self.prediction[group][db_const.percentage_local_original_host].append(percentage)
            self.prediction[group][db_const.population_local_original_host].append(population)
        return
    ...
    def append_game_id_and_type(self, row_content, date, group=None):
        ...
        if group:
            self.logger.debug(
                'current column size of {}: {}'.format(db_const.gamble_id,
                                                       len(self.prediction[group][db_const.gamble_id])))
        ...
        # game_data table
            ...
            self.logger.debug(
                'current column size of {}: {}'.format(db_const.gamble_id, len(self.game_info[db_const.gamble_id])))
    ...
    def append_game_time(self, row_content):
        self.logger.info(
            'current column size of {}: {}'.format(db_const.play_time, len(self.game_info[db_const.play_time])))
    ...
    def append_score(self, row_content):
        self.logger.info(
            'current column size of {}: {}'.format(db_const.guest_score,
                                                   len(self.game_info[db_const.guest_score])))
        self.logger.info(
            'current column size of {}: {}'.format(db_const.host_score, len(self.game_info[db_const.host_score])))
    ...
    def append_team_name(self, row_content):
        self.logger.info(
            'current column size of {}: {}'.format(db_const.guest_id, len(self.game_info[db_const.guest])))
        self.logger.info(
            'current column size of {}: {}'.format(db_const.host_id, len(self.game_info[db_const.host])))
        ...
        guest_abbreviate = cl_const.team_name_mapping[
            guest] if guest in cl_const.team_name_mapping.keys() else guest
        host_abbreviate = cl_const.team_name_mapping[
            host] if host in cl_const.team_name_mapping.keys() else host
    ...
    def append_point_spread_info(self, row_content, guest_row):
        ...
        if len(spread_info) > 1:
            ...
            national_spread_point = -int(national_spread_point) if national_spread_from == db_const.guest else int(
                national_spread_point)
            self.game_info[db_const.national_host_point_spread].append(national_spread_point)
            self.game_info[db_const.win_if_meet_spread_point].append(hit_result)
            self.game_info[db_const.response_ratio_if_hit_spread_point].append(hit_percentage / 100)
        ...
        if guest_row:
            ...
            self.game_info[db_const.local_host_point_spread].append(float(local_host_spread_point))
            ...
            self.game_info[db_const.local_total_point_threshold].append(float(local_total_point))
            self.game_info[db_const.local_total_point_threshold_response_ratio].append(float(total_point_response))
    ...
    def append_total_point_info(self, row_content):
        ...
        self.game_info[db_const.national_total_point_threshold].append(float(national_total_point))
        pass
    ...
    def append_response_ratio_info(self, row_content, guest_row):
        ...
        if guest_row:
            ...
            self.game_info[db_const.local_origin_host_response_ratio].append(float(local_origin_host_response_ratio))
    ...
    def get_url(self, date, member_type=1):
        return self.config['crawler']['urlPattern'].format(game_type=gbl_const.game_type_map[self.game_type],
                                                           game_date=date,

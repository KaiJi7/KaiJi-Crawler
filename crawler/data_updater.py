...
class DataUpdater(object):
    ...
    def update_db(self):
        ...
        for game_type, crawl_ranges in self.get_crawl_range().items():
            ...
            for crawl_range in crawl_ranges:
                self.logger.debug('start crawler with date range: {}-{}'.format(crawl_range.begin, crawl_range.end))
                Crawler(game_type=game_type).start_crawler(start_date=str(crawl_range.begin),
                                                           end_date=str(crawl_range.end))
    ...
    def get_crawl_range(self):
        ...
        for game_type, season_info in self.game_season.items():
            self.logger.debug('start date crawl range of game type: {}'.format(game_type))
            # get latest game date of the game_type
            latest_game = min(max(flatten(season_info).values()),
                              int(datetime.datetime.strftime(datetime.datetime.now(), '%Y%m%d')))
            if game_type in self.get_latest_record() and self.get_latest_record()[game_type] < latest_game:
                self.logger.debug('game data was out of date, the latest record in db is: {}, '
                                  'the latest game is: {}'.format(self.get_latest_record()[game_type], latest_game))
                crawl_range[game_type] = self.date_overlap(self.get_latest_record()[game_type], latest_game, game_type)
    ...
    def date_overlap(self, latest_record, latest_game, game_type):
        ...
        for season in self.game_season[game_type].keys():
            if latest_record < self.game_season[game_type][season]['start']:
                self.logger.debug('start date of the season: {} after latest record: {}'.format(season, latest_record))
                overlap.append(CrawlRange(min(self.game_season[game_type][season]['start'], latest_game),
                                          min(self.game_season[game_type][season]['end'], latest_game)))
            elif latest_record < self.game_season[game_type][season]['end']:
                self.logger.debug('end date of the season: {} after latest record: {}'.format(season, latest_record))
                overlap.append(CrawlRange(latest_record, min(self.game_season[game_type][season]['end'], latest_game)))
            else:
                self.logger.debug('the latest record after the season, no overlap of this season')
        return overlap
